...
export default function HomePage() {
  ...
  const handleDrop = (e) => {
    ...
    if (validFiles.length < droppedFiles.length) setError("Only image files are allowed.");
    else setError("");
  ...
  const handleFileChange = (e) => {
    ...
    if (validFiles.length < selected.length) setError("Only image files are allowed.");
    else setError("");
    setFiles((prev) => [...prev, ...validFiles]);
    setPreviews((prev) => [...prev, ...validFiles.map(file => URL.createObjectURL(file))]);
  };
  ...
  const handleSubmit = async () => {
    ...
    console.log("🧠 Calling FastAPI backend at http://127.0.0.1:8000/generate");
    const data = await res.json();
    navigate("/result", {
      state: {
        layoutData: data.layout_raw,
        previews,
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6">
      <h2 className="text-3xl font-semibold text-gray-800 mb-2">Vistack Editor</h2>
      <p className="text-gray-500 mb-6 text-center max-w-md">Upload 2–6 images. We'll generate a layout instantly.</p>
      {error && (
        <div className="mb-4 px-4 py-2 bg-red-100 text-red-600 rounded border border-red-300">{error}</div>
      )}
      <div
        onDrop={handleDrop}
      ...
      >
        {previews.map((url, i) => (
          <div key={i} className="relative group">
            <img src={url} alt={`preview-${i}`} className="w-28 h-40 object-cover rounded shadow" />
            <button
